...
renameClusterServer <- function(input, output, session, inSCE = NULL, isCluster = FALSE, clustRedDim = NULL, clustVisRes = NULL, clustVisCol = NULL) {
  ...
  observeEvent(inSCE$counts, {
    req(inSCE$counts)  # Ensure counts data exists
    req(colData(inSCE$counts))  # Ensure colData exists
    req(colnames(colData(inSCE$counts)))  # Ensure colData has column names
    ...
    if (is.null(inSCE$counts)) {
      ...
      output$textFieldsContainer <- renderUI({
        req(input$ClusterLabelChoices)  # Ensure a choice is selected
        req(colData(inSCE$counts))  # Ensure colData exists
        req(input$ClusterLabelChoices %in% colnames(colData(inSCE$counts)))  # Ensure the selected column exists
        
        # Dynamically create UI elements for each unique factor
        selectInputs <- lapply(unique(colData(inSCE$counts)[[input$ClusterLabelChoices]]), function(factor) {
          div(
            ...
            textInput(
              ...
              label = NULL,
              value = text_input_values[[paste0("textField", factor)]],  # Set initial value
              width = "300px"
            )
          )
        })
        tagList(selectInputs)  # Return the dynamically created UI elements
      })
    }
    if (!is.null(inSCE$counts) && !is.null(colnames(colData(inSCE$counts)))) {
      options <- colnames(colData(inSCE$counts))
